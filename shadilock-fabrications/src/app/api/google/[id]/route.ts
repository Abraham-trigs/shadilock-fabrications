...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json(
      { error: "Failed to fetch file", details: error instanceof Error ? error.message : "Unknown error" },
      { status: 500 }
    );
  }
}
...
// --- PUT update specific file (rename) ---
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json(
      { error: "Failed to update file", details: error instanceof Error ? error.message : "Unknown error" },
      { status: 500 }
    );
  }
}
